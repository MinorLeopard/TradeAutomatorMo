...
def webhook():
    ...
    try:
        ...
        ltp=vo['d'][0]['v']['lp']
        stop_loss = (0.10 * ltp)
        take_profit = (0.25 * ltp)#variable as per risk appetite
        if timeframe==15:
            take_profit=(0.50 * ltp)
        data = {
              "symbol":str(get_current_expiry(symbol,order_type,price)),
              "qty":quantity,
              "type":2,
              "side":1,
              "productType":"BO",#Margin orders dont take profit and stop loss
              "limitPrice":0,
              "stopPrice":0,
              "validity":"DAY",
              "disclosedQty":0,
        ...
        create_csv_for_day(current_date)
        write_order_to_csv(current_date, data,order_type, buy_response,symbol)
        update_buy_price_and_timestamp(current_date, order_id, buy_price, timestamp)
